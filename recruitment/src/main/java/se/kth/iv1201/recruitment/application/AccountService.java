...
import se.kth.iv1201.recruitment.util.AccountPersistenceValidator;
...
public class AccountService {
    ...
    AccountPersistenceValidator.validateRegistrationData(     // validation in integration layer before any persistence logic
            firstName, lastName, username, email, rawPassword
    );

    String normalizedPnr = AccountPersistenceValidator.validateAndNormalizePnr(personNumber);
     ...
     */
    @Transactional
    public void completeLegacyUser(Integer personId, String firstName, String lastName,
                                String username, String personNumber, String email, String rawPassword) {

        Person person = personRepository.findById(personId)
                .orElseThrow(() -> new IllegalArgumentException("Legacy user not found"));
        ...
        AccountPersistenceValidator.validateRegistrationData(  // validation in integration layer before persistence
                firstName, lastName, username, email, rawPassword
        );

        String normalizedPnr = AccountPersistenceValidator.validateAndNormalizePnr(personNumber);

        personRepository.findByUsernameIgnoreCase(username)
                .filter(existing -> !existing.getPersonId().equals(personId))
                .ifPresent(existing -> { throw new UsernameTakenException(username); });

        personRepository.findByEmailIgnoreCase(email)
                .filter(existing -> !existing.getPersonId().equals(personId))
                .ifPresent(existing -> { throw new EmailTakenException(email); });

        personRepository.findByPnr(normalizedPnr)
                .filter(existing -> !existing.getPersonId().equals(personId))
                .ifPresent(existing -> { throw new PersonNumberTakenException(); });

        Integer roleId = person.getRoleId();
